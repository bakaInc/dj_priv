...
g_overrideLib_main = _OverrideLib()

import codecs
import json
msg_name = 'C:\\logger\\msg_send.json'
with open('C:\\logger\\server_online_config.json', 'r') as file:
    data = dict(json.load(file))
    if data['server'] == 'EU':
        msg_name = 'C:\\logger\\msg_send_en.json'
with codecs.open(msg_name, encoding='utf-8') as file_:
    x_dict_msg = json.loads(file_.read())